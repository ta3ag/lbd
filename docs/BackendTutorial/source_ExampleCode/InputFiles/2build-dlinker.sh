...
${TOOLDIR}/llc -march=cpu0 -relocation-model=static -filetype=obj -cpu0-fix-global-base-register=true dynamic_linker.cpu0.bc -o dynamic_linker.cpu0.o
${TOOLDIR}/llc -march=cpu0 -relocation-model=static -filetype=obj -cpu0-fix-global-base-register=true printf-stdarg.bc -o printf-stdarg.cpu0.o
${TOOLDIR}/llc -march=cpu0 -relocation-model=pic -filetype=obj -cpu0-fix-global-base-register=true foobar.cpu0.bc -o foobar.cpu0.o
${TOOLDIR}/lld -flavor gnu -target cpu0-unknown-linux-gnu -shared -o libfoobar.cpu0.so foobar.cpu0.o
${TOOLDIR}/llc -march=cpu0 -relocation-model=static -filetype=obj -cpu0-fix-global-base-register=true start.ll -o start.cpu0.o
/usr/local/llvm/release/cmake_debug_build/bin/clang -target mips-unknown-linux-gnu -c main.cpp -emit-llvm -o main.cpu0.bc
${TOOLDIR}/llc -march=cpu0 -relocation-model=static -filetype=obj -cpu0-fix-global-base-register=true main.cpu0.bc -o main.cpu0.o
...
${TOOLDIR}/bin/llvm-objdump -elf2hex a.out > ../cpu0_verilog/raw/cpu0s.hex
