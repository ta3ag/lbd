//===- lib/ReaderWriter/ELF/Cpu0/Cpu0TargetInfo.cpp -------------------===//
//
//                             The LLVM Linker
//
...
public:
  StaticGOTPLTPass(const elf::Cpu0TargetInfo &ti) : GOTPLTPass(ti) {}
...
public:
  DynamicGOTPLTPass(const elf::Cpu0TargetInfo &ti) : GOTPLTPass(ti) {}
...
void elf::Cpu0TargetInfo::addPasses(PassManager &pm) const {
...
elf::Cpu0TargetInfo::relocKindFromString(StringRef str) const {
...
ErrorOr<std::string>
elf::Cpu0TargetInfo::stringFromRelocKind(Reference::Kind kind) const {
  switch (kind) {
  LLD_CASE(R_CPU0_NONE)
  LLD_CASE(R_CPU0_16)
  LLD_CASE(R_CPU0_32)
