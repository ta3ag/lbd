...
dynamic linker are not exist at this point. We have check this implementation 
...
.. code-block:: bash
  ...
  00400720 <_Z3barv@plt>:
    ...
    400728:	jr	$6
    40072c:	addiu	$9,$8,0x0acc
  
  00400730 <_Z3fooii@plt>:
    400730:	lui	$8,0x41
    400734:	ld	$6,0x0ado($8)
    400738:	jr	$6
    40073c:	addiu	$9,$8,0x0ado
  ...
  004009f0 <.CPU0.stubs>:
...
_Z3fooii@plt, the contents of address .dynsym+0x08 include the 
...
.. table:: registers changed for call dynamic link function _Z3fooii()
  ...
  .dynsym+0x08                         (libfoobar.so, offset, length) about _Z3fooii     -
  -32752(gp)                           point to dynamic_linker                           -
  $8                                   the next instruction of _Z3fooii()                -
  $9                                   the contents of 0x410ad0                          -
  $6 (at the end of _Z3fooii@plt)      point to CPU0.stubs0                              -
  $6 (at the end of CPU0.stubs)        point to dynamic_linker                           -
  ===================================  ================================================  ========================================

Explains it as follows,

1. As you can see, the first time, function call, a = foo(1,2), is implemented 
   by instruction "ld $7, %call24(_Z3fooii@plt)($gp)", "add $6, $zero, $7" and 
   "jalr $6". Remember, .dynsym+0x08 contains information (libfoobar.so, 
   offset, length) whick set at link to dynamic shared library.
...
3. After this plt instructions executed, it jump to CPU0.stubs since $6 = the 
...
4. After CPU0.stubs is executed, register $8 = 0x004008a4 which point to the 
   next instruction of step 1.

5. Dynamic linker look into register $9 which value is 0x08. It ask OS for 
   the caller process address information of .dynsym + offset 0x08. This 
   information (libfoobar.so, offset, length) tells where can get _Z3fooii 
   function body. Dynamic linker load _Z3fooii() function body to an available 
   address by asking OS. 

6. Set the contents of address 0x410ad0 to the entry address of _Z3fooii in 
   memory.

7. Execute jr $8. It jump to the next instruction of "a = _Z3fooi();" in caller.
...
at step 6 of :num:`Figure #elf-f2`.
...
To support dynamic funciton, Cpu0 backend generate jalr instead of jal.
...
interest and already know how to track via lldb or gdb.
