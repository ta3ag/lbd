...
template <class ELFT> void DefaultLayout<ELFT>::assignSectionsToSegments() {
...
#if 1 // cschen debug
  for (auto si : _sections) {
    StringRef cc = si->name();
    uint64_t dd = si->order();
    uint64_t ee = si->ordinal();
    uint64_t aa = si->fileOffset();
    uint64_t bb = si->fileSize();
    llvm::outs() << cc << " " << dd << " " << ee << " " << aa << " " << bb << "\n";
  }
    llvm::outs() << "!1.1\n\n";
#endif
  ...
  std::stable_sort(_sections.begin(), _sections.end(),
    ...
    if (A->name() == ".rodata" && B->name() == ".text")
      return false;
    else
      return A->order() < B->order();
  });
#if 1 // cschen debug
  ChunkIter sa = _sections.begin();
  ChunkIter sb = _sections.begin();
  int rodata_order = 0, text_order = 0, order = 0;
  for (ChunkIter si = _sections.begin(); si != _sections.end(); si++) {
    if ((*si)->name() == ".rodata") {
      sa = si;
      rodata_order = order;
      order++;
  ...
  if ((*sa)->name() == ".rodata" && (*sb)->name() == ".text" && (rodata_order < text_order)) {
    std::swap(*sa, *sb);
    llvm::outs() << "text_order " << text_order << "rodata_order " << rodata_order << "\n";
    llvm::outs() << "swap section .rodata and .text\n\n";
  }
#endif
#if 1
  for (auto si : _sections) {
...
DefaultLayout<ELFT>::assignVirtualAddress() {
  ...
  for (auto si : _sections) {
    ...
    llvm::outs() << "!!\n\n";
  for (auto si : _mergedSections) {
    StringRef cc = si->name();
    uint64_t dd = si->virtualAddr();
    uint64_t ee = si->ordinal();
    uint64_t aa = si->fileOffset();
    uint64_t bb = si->fileSize();
    llvm::outs() << cc << " " << dd << " " << ee << " " << aa << " " << bb << "\n";
