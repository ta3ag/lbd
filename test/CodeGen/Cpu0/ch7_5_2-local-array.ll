...
; STATIC:  lui	$[[T0:[0-9]+]], %hi($_ZZ4mainE1a)
; STATIC:  addiu	$[[T1:[0-9]+]], $[[T0]], %lo($_ZZ4mainE1a)
; STATIC:  ld	$[[T2:[0-9]+]], 8($[[T1]])
; STATIC:  st	$[[T2]], {{[0-9]+}}(${{[fs]p}})
; STATIC:  ld	$[[T2:[0-9]+]], 4($[[T1]])
; STATIC:  st	$[[T2]], {{[0-9]+}}(${{[fs]p}})
; STATIC:  ld	$[[T2:[0-9]+]], 0($[[T1]])
; STATIC:  st	$[[T2]], {{[0-9]+}}(${{[fs]p}})

; PIC:  ld	$[[T0:[0-9]+]], %got($_ZZ4mainE1a)($gp)
; PIC:  addiu	$[[T1:[0-9]+]], $[[T0]], %lo($_ZZ4mainE1a)
; PIC:  ld	$[[T2:[0-9]+]], 8($[[T1]])
; PIC:  st	$[[T2]], {{[0-9]+}}(${{[fs]p}})
; PIC:  ld	$[[T2:[0-9]+]], 4($[[T1]])
; PIC:  st	$[[T2]], {{[0-9]+}}(${{[fs]p}})
